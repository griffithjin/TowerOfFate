...
def initialize_demo_data():
    ...
    for i, name in enumerate(team_names):
        captain_id = f"demo_captain_{i+1}"
        captain_name = f"队长{i+1}号"
        
        result = team_system.create_team(captain_id, captain_name, name)
        if result['success']:
            ...
            for j in range(team_size):
                player_id = f"demo_player_{i}_{j}"
                player_name = f"{name[2:]}成员{j+1}"
                team_system.join_team(player_id, player_name, team_id)
            
            # 随机设置一些战绩
            team = team_system.teams[team_id]
            team.wins = random.randint(5, 50)
            team.losses = random.randint(2, 20)
    ...
    for i in range(80):
        player_id = f"online_player_{i+1}"
    ...
    # 存储在线玩家列表
    team_system.online_players = online_players
    
    print(f"\n📊 演示数据初始化完成:")
    print(f"  - 团队数量: {len(created_teams)}")
    print(f"  - 在线玩家: {len(online_players)}")
    print(f"  - 总玩家数: {sum(len(team_system.teams[tid].players) for tid in created_teams) + len(online_players)}")
...
# 获取在线玩家列表
def get_online_players(limit: int = 20) -> List[Dict]:
    """获取在线玩家列表"""
    if hasattr(team_system, 'online_players'):
        return random.sample(team_system.online_players, 
                           min(limit, len(team_system.online_players)))
    return []


# 获取推荐团队
def get_recommended_teams(player_id: str, limit: int = 5) -> List[Dict]:
    """获取推荐团队"""
    if player_id in team_system.player_team:
        return []  # 已在团队中
    
    # 返回人数未满的团队
    available_teams = []
    for team in team_system.teams.values():
        if len(team.players) < 10:  # 团队人数上限10人
            available_teams.append(team_system._format_team_info(team))
    
    # 随机返回
    return random.sample(available_teams, min(limit, len(available_teams)))
